...
namespace NNC {
	...
	constexpr auto  PawnCapture = (1 << 2);	// PseudoLegal move table
	...
	constexpr Bitboard FB = Bitboard(0x0202020202020202ULL);	// B file
	...
	inline Bitboard DiagD_bb(Square sq) {
		SquareOk(sq); return Bitboard(diagD_BB[sq]);
	}
	inline Bitboard DiagU_bb(Square sq) {
		SquareOk(sq); return Bitboard(diagU_BB[sq]);
	}
